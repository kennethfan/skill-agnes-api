#!/usr/bin/env python3
"""
Pipeline: 历史上的今天 — 三阶段视频制作管线

Stages:
  1. 场景图生成 — AGNES t2i（并行 5 worker, 重试 3 次）
  2. 素材准备 — Pillow 渲染字幕帧 + edge-tts 旁白音频
  3. 视频合成 — ffmpeg concat 合成最终 MP4

使用:
  python3 scripts/pipelines/today-in-history-pipeline.py --date 0723 --yaml today-in-history-2026-07-23.yaml
  python3 scripts/pipelines/today-in-history-pipeline.py --date 0723 --yaml ... --clean
  python3 scripts/pipelines/today-in-history-pipeline.py --date 0723 --yaml ... --force-stage 2
"""
...
def _image_name(date: str, scene_idx: int, ts: int) -> str:
    """comic-scene-{date}-{NN}-{ts}.png"""
    return f"comic-scene-{date}-{scene_idx + 1:02d}-{ts}.png"


def _image_pattern(date: str) -> re.Pattern:
    return re.compile(rf"comic-scene-{re.escape(date)}-(\d{{2}})-.+\.png$")
...
def _agnes_generate(prompt: str, api_key: str) -> str:
    """Call AGNES t2i, return download URL."""
    body = {
        "model": AGNES_MODEL,
        "prompt": prompt.strip(),
        "size": AGNES_SIZE,
...
def scan_scene_images(date: str) -> dict[int, Path]:
    """Return {scene_num: latest_file_path} for given date."""
    pat = _image_pattern(date)
    by_scene: dict[int, list[tuple[int, Path]]] = {}
    for f in sorted(IMAGES_DIR.glob(f"comic-scene-{date}-*.png")):
        m = pat.search(f.name)
        if not m:
            continue
        sn = int(m.group(1))
...
def stage1_generate(date: str, scenes: list[dict]) -> int:
    """Generate scene images via AGNES t2i. Returns number of failures."""
    print("\n" + "=" * 60)
    print(f"🎨 Stage 1: Scene Image Generation ({date})")
    print("=" * 60)

    api_key = _load_agnes_key()
    print(f"🔑 Key loaded ({api_key[:8]}...{api_key[-4:]})")

    existing = scan_scene_images(date)
    ...
    def _work_one(idx: int, sc: dict) -> tuple[int, str | None, str | None]:
        ...
        for attempt in range(1, AGNES_RETRIES + 1):
            try:
                ...
                fname = _image_name(date, idx, int(time.time()))
...
def _render_subtitle(img: Image.Image, text: str) -> Image.Image:
    """Bake narration text + semi-transparent bar into image. No character prefix."""
...
def _create_title_card(title: str, date_str: str, bg_img: Image.Image | None) -> Image.Image:
    """Dark overlay + centred title + date + tagline."""
    ...
    sw = draw.textbbox((0, 0), date_text, font=sf)[2]
    sx = (bg.width - sw) // 2
    sy = ty + TITLE_FONT_SIZE + 28
    draw.text((sx + 2, sy + 2), date_text, font=sf, fill=(0, 0, 0))
    draw.text((sx, sy), date_text, font=sf, fill=(200, 200, 200))
...
def stage2_prepare(
    date: str, title: str, scenes: list[dict], temp_dir: Path,
) -> list[tuple[Path, Path, float]]:
    """Render frames + TTS audio. Returns segments for Stage 3."""
    print("\n" + "=" * 60)
    print(f"🎬 Stage 2: Asset Preparation ({date})")
    print("=" * 60)

    temp_dir.mkdir(parents=True, exist_ok=True)
    image_map = scan_scene_images(date)
    ...
    title_img = _create_title_card(title, date, first_img)
    ...
    for si, sc in enumerate(scenes):
        ...
        for di, dlg in enumerate(dlg_list):
            ...
            if not ap.exists():
                _generate_audio(text, NARRATOR_VOICE, ap)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="历史上的今天 — 三阶段视频管线")
    p.add_argument("--date", required=True, help="日期 MMdd 格式，如 0723")
    p.add_argument("--yaml", required=True, help="YAML 剧本路径（today-in-history-YYYY-MM-DD.yaml）")
...
def main() -> None:
    ...
    if not re.match(r"^\d{4}$", date):
        print(f"❌ Invalid date format: {date!r}, expected MMdd")
        sys.exit(1)
    ...
    title: str = data.get("title", f"历史上的今天{date}")
    scenes: list[dict] = data.get("scenes", [])
    if not scenes:
        print("❌ No scenes in YAML")
        sys.exit(1)
    print(f"📖 {len(scenes)} scenes, title={title!r}")

    # --- Checkpoint key ---
    cp_key = f"history-{date}"

    # --- Output ---
    output = (
        Path(args.output)
        if args.output
        else VIDEOS_DIR / "历史上的今天" / f"历史上的今天{date}.mp4"
    )
    print(f"📁 Output: {output}")

    temp_dir = TEMP_BASE / f"pipeline_{date}"
    ...
    if skip_s1:
        ...
        failed = stage1_generate(date, scenes)
        if failed:
    ...
    if skip_s2:
        ...
        segments = stage2_prepare(date, title, scenes, temp_dir)
